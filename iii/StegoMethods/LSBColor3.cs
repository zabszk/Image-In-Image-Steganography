...
public class LSBColor3 : StegoImageMethod
{
	public override string Name => "Least Significant 3 Bits - Color";

	public override string FileSuffix => "lsb3";
	...
	public override async Task<Bitmap> Encode(Bitmap coverBmp, Bitmap hiddenBmp, StegoParameters parameters)
	{
		...
		{
			Random rng = new();
			AddNoisePixelRows(coverBmp, outputBmp, x, y, 255, 0b_1111_1000, 0b_1111_1000, 0b_1111_1000, rng);
		}
	...
	public override Task<Bitmap?> Decode(Bitmap stegoBmp)
	{
		...
		for (x = 0; x < stegoBmp.Width; x++)
		{
			for (y = 0; y < stegoBmp.Height; y++)
			{
				...
				int tempg = (stegoPixelColor.G & 0b_0000_0111) << 5;
				int tempb = (stegoPixelColor.B & 0b_0000_0111) << 5;
				Color newColor = Color.FromArgb(tempr, tempg, tempb);
				outputBmp.SetPixel(x, y, newColor);
			}
		}
