...
public class LSBColor2 : StegoImageMethod
{
	public override string Name => "Least Significant 2 Bits - Color";

	public override string FileSuffix => "lsb2";
	...
	public override async Task<Bitmap> Encode(Bitmap coverBmp, Bitmap hiddenBmp, StegoParameters parameters)
	{
		...
		{
			Random rng = new();
			AddNoisePixelRows(coverBmp, outputBmp, x, y, 255, 0b_1111_1100, 0b_1111_1100, 0b_1111_1100, rng);
		}
	...
	public override Task<Bitmap?> Decode(Bitmap stegoBmp)
	{
		...
		for (x = 0; x < stegoBmp.Width; x++)
		{
			for (y = 0; y < stegoBmp.Height; y++)
			{
				...
				int tempg = (stegoPixelColor.G & 0b_0000_0011) << 6;
